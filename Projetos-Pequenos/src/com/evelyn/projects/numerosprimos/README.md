...
## 📖 Sobre
Esta aplicação irá receber um número e dirá se ele é primo ou não, imprimindo os números pelos quais ele é divisível caso não seja primo. Foi Feita utilizando apenas a linguagem `Java` e a IDE `VSCODE`.
...
## 📖 About
This application receives a number and determines whether it is prime or not, printing the numbers by which it is divisible if it is not prime. It was made using the language `Java` and IDE `VSCODE`.
