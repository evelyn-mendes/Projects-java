...
## 📖 Sobre
Esta aplicação receberá N quantidade de turmas e N quantidades de alunos por turmas, não podendo ultrapassar de 40 alunos por turma. E, após, imprime na tela a média de alunos por turmas. Foi feita utilizando apenas a linguagem `Java` e a IDS `VSCODE`.
...
## 📖 About
This application will receive N quantities of classes and N quantities of students per classes. And then, it prints on screen the average number of students per class. It was made using the language `Java` and the `VSCODE` IDE.
