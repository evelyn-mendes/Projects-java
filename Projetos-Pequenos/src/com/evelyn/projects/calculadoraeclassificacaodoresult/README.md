...
## 📖 Sobre
Este aplicativo armazena dois números e permite ao usuário realizar uma operação matemática (somar, subtrair, multiplicar ou dividir). Depois imprime o resultado, e se o resultado é par ou ímpar e se ele é decimal ou inteiro, positivo ou negativo. Foi feito apenas utilizando a linguagem `Java` e a IDE `VSCODE`.
...
This application stores two numbers and allows the user to perform a mathematical operation (addition, subtraction, multiply or divide). Then print the result, and whether the result is even or odd and whether it is a decimal or integer, positive or negative. It was made using the language `Java` and the `VSCODE` IDE.
